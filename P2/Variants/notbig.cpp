...
typedef ttmath::Int<10> ll;
typedef ttmath::Big<10,5> bigdouble;

bigdouble biglog(ll val)
{
	bigdouble res;
	res.Log(val,2);
	return res;
...
void mult(ll *a, ll *b, ll *c, ll len)
{
	N = 2 * len;
	while(N & (N - 1))
		++N;
...
bool AKS(ll N)
{
	...
	bigdouble t = biglog(N);
	t*=t;
	ll maxk = ttmath::Ceil(t);
	cout << "maxk = " << maxk << endl;
	bool nextR = true;
	ll r,k,temp,a,gcdan;
	for(r=2;nextR;r++)
	...
	ll maxa = ttmath::ceil(bigsqrt(phi(r))*biglog(N));
	cout << "maxa = " << maxa << endl;
